...
TravellingSalesmanProblem::TravellingSalesmanProblem(const int problem_size, float* cities,
    ...
    sizeWorkingSet += sizeof(Int) * problem_size * population_count; // temp population matrix
    sizeWorkingSet += population_count * sizeof(Real); // fitness vector
    sizeWorkingSet += population_count * sizeof(Real); // fitness weight vector
    cout << "size of working set is " << sizeWorkingSet << endl;
}
...
TravellingSalesmanProblem::~TravellingSalesmanProblem() {
    ...
    cout << "number of crossovers was " << numCrossovers << endl;
    
    double accRuntimeRank = (double)accumulate(rankRuntimes.begin(), rankRuntimes.end(), (double)0.0) / (double)1e9;
    double accRuntimeBreed = (double)accumulate(breedRuntimes.begin(), breedRuntimes.end(), (double)0.0) / (double)1e9;
    double accRuntimeMutate = (double)accumulate(mutateRuntimes.begin(), mutateRuntimes.end(), (double)0.0) / (double)1e9;
    cout << "--------------------------" << endl;
    cout << "accumulated runtime rank: " << accRuntimeRank << "s" << endl;
    cout << "accumulated runtime breed: " << accRuntimeBreed << "s" << endl;
    cout << "accumulated runtime mutate: " << accRuntimeMutate << "s" << endl;
    ...
    //double accSampleFromCorrected = (double)accumulate(sampleFromCorrected.begin(), sampleFromCorrected.end(), 0) / (double)1e9;
    //cout << "--------------------------" << endl;
    ...
    //cout << "accumulated runtime sample from corrected: " << accSampleFromCorrected << "s" << endl;
    
    cout << "--------------------------" << endl;
    cout << "(total accumulated runtime: " << (accRuntimeRank+accRuntimeBreed+accRuntimeMutate) << ")" << endl;
    cout << "runtime SOLVE: " << runtimeSolve / (double)1e9 << endl;
    
    string rankRuntimesFile = "rankRuntimesFile.csv";
    string breedRuntimesFile = "breedRuntimesFile.csv";
    string mutateRuntimesFile = "mutateRuntimesFile.csv";
    std::ofstream outputFileStream;
    outputFileStream.open(rankRuntimesFile);
    outputFileStream << "runtime_rank" << "\n"; // title
    for(int idx = 0; idx < rankRuntimes.size(); idx++) {
        outputFileStream << (long)rankRuntimes[idx];
        outputFileStream << "\n";
    }
    outputFileStream.close();
    outputFileStream.open(breedRuntimesFile);
    outputFileStream << "runtime_breed" << "\n"; // title
    for(int idx = 0; idx < breedRuntimes.size(); idx++) {
        outputFileStream << (long)breedRuntimes[idx];
        outputFileStream << "\n";
    }
    outputFileStream.close();
    outputFileStream.open(mutateRuntimesFile);
    outputFileStream << "runtime_mutate" << "\n"; // title
    for(int idx = 0; idx < mutateRuntimes.size(); idx++) {
        outputFileStream << (long)mutateRuntimes[idx];
        outputFileStream << "\n";
    }
    outputFileStream.close();
#endif
...
void TravellingSalesmanProblem::evolve(const int rank) {
    ...
    tEnd = myClock.now();
    delta = std::chrono::duration_cast<hrNanos>(tEnd - tStart).count();
    mutateRuntimes.push_back(delta);
#endif
...
/*#ifdef microbenchmark_breed
    ...
    int delta;
    tStart = myClock.now();
    hrTime tStartIntermediate, tEndIntermediate;
    ...
    /*if(sizeof(Int) == 4) { // 32-bit version
        ...
        for(; popIdx < elite_size; popIdx++) {
            int currIndividual = ranks[popIdx];
            int geneIdx = 0;
            for(; geneIdx <= problem_size - 16; geneIdx = geneIdx + 16) {
                geneSegSIMD = _mm256_load_si256((__m256i *)&POP(currIndividual, geneIdx));
                _mm256_store_si256((__m256i *)&temp_population[popIdx*problem_size + geneIdx], geneSegSIMD);
            }
            for(; geneIdx < problem_size; geneIdx++) {
...
/*#ifdef microbenchmark_breed
        tStartIntermediate = myClock.now();
#endif*/
        int rand1 = dist(gen);
        int rand2 = dist(gen);
        
/*#ifdef microbenchmark_breed
        tEndIntermediate = myClock.now();
        deltaIntermediate = std::chrono::duration_cast<hrNanos>(tEndIntermediate - tStartIntermediate).count();
        sampleFromCorrected.push_back(deltaIntermediate);
#endif*/
